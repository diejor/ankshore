...
func load_state(save_slot: String) -> Error:
	if not ResourceLoader.exists(save_slot, "DictionaryResource"):
		push_warning("No `DictionaryResource` file found at path: 
			%s" % save_slot)
		return ERR_FILE_NOT_FOUND

	var res := ResourceLoader.load(save_slot, "DictionaryResource")
	...
	assert(dict_res != null, 
		"Loaded resource at %s is not a DictionaryResource." % save_slot)
	if dict_res == null:
